...
class TestCharacterScreen(unittest.TestCase):
    ...
    def test_character_name_already_taken_response(self):
        """Test character_name_already_taken packet is sent when name exists."""
        ...
        # Create mocks
        mock_logger = MagicMock(spec=ServerLogger)
        mock_player_manager = MagicMock(spec=PlayerManager)
        mock_parent_node = MagicMock()
        mock_socket = MagicMock()

        # Create client handler
        client_handler = ClientHandler(
            mock_logger, mock_player_manager, mock_parent_node
        )
        player_index = 0x1234

        # Mock the character database to return that name exists
        with patch.object(
            client_handler.character_db, "character_name_exists", return_value=True
        ):
            with patch.object(
                client_handler.state_manager, "get_user_id", return_value="testuser"
            ):
                with patch.object(
                    ServerPackets, "character_name_already_taken"
                ) as mock_packet:
                    with patch.object(SocketUtils, "send") as mock_send:
                        with patch.object(
                            client_handler, "_cleanup_client"
                        ) as mock_cleanup:
                            with patch.object(
                                client_handler,
                                "_decode_character_name",
                                return_value="ExistingName",
                            ):

                                # Set up mock returns
                                mock_packet.return_value = b"name_taken_packet"
                                mock_send.return_value = True

                                # Create test packet data for character creation
                                test_data = bytearray(30)
                                test_data[0] = 30  # Packet length
                                test_data[13] = 0x08
                                test_data[14] = 0x40
                                test_data[15] = 0x80
                                test_data[16] = 0x05
                                test_data[17] = 8  # Character slot

                                # Call the character creation method
                                result = client_handler._create_character_from_packet(
                                    test_data, "testuser", 1, player_index
                                )

                                # Verify that creation failed
                                self.assertFalse(result)

                                # Verify that character_name_exists was called
                                char_db = client_handler.character_db
                                char_db.character_name_exists.assert_called_once_with(
                                    "ExistingName"
                                )

        # Test the full character screen flow when name is taken
                    ...
                    with patch.object(SocketUtils, "send") as mock_send:
                        with patch.object(
                            ...
                            with patch.object(
                                ...
                                # Call _wait_for_character_screen
                                result = client_handler._wait_for_character_screen(
                                ...
                                # Verify that the correct packet was created and sent
                                mock_packet.assert_called_once_with(player_index)
                                expected_msg = (
                                    f"Sent character name already taken to player "
                                    f"0x{player_index:04X}: "
                                    f"{b'name_taken_packet'.hex().upper()}"
                                )
                                ...
                                # Verify that cleanup was called (connection closed)
