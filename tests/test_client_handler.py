...
from server.client_state_manager import ClientState
...
class TestClientHandler(unittest.TestCase):
    ...
    def test_start_handling_accept_connection(self, mock_socket_class):
        ...
        self.mock_player_manager.add_player.assert_called_once_with(player_index, address)
        ...
        self.mock_logger.debug.assert_any_call(
            f"Sent init packet to player 0x{player_index:04X}: {INIT_PACKET.hex()}"
        )
    ...
    def test_reject_connection_max_players(self):
        ...
        mock_server_socket.accept.return_value = (mock_client_socket, address)
        self.mock_player_manager.get_next_player_index.return_value = None  # Max players
        ...
        self.mock_logger.warning.assert_called_with(
            f"Rejecting connection from {address}: Maximum players reached"
        )
    ...
    def test_state_manager_integration(self):
        ...
        with patch.object(self.client_handler.state_manager, 'add_client') as mock_add:
            with patch.object(self.client_handler.state_manager, 'transition_to_init_ready') as mock_transition:
