...
from .exceptions import character_screen_exception


class character_screen_handler:
	...
	def __init__(self, character_db: character_database):
		...
		self.character_creator = character_creation_handler(character_db)
	...
	def wait_for_character_screen_interaction(
		...
		Raises:
			character_screen_exception: If character screen interaction fails or character
				is deleted
		"""
		try:
			while True:
				...
				if not data:
					raise character_screen_exception(
				...
				if user_id:
					...
					try:
						...
						success_packet = server_packets.character_name_check_success(
							player_index
						)
						...
						name_taken_packet = server_packets.character_name_already_taken(
							player_index
						)
						send_packet_success = server_socket_utils.send_packet_or_cleanup(
							client_socket,
		...
		except character_screen_exception:
			raise
