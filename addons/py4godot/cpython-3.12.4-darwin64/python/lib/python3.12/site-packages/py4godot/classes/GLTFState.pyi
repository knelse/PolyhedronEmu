from py4godot.utils.VariantTypeWrapper4 import *
import py4godot.classes.Object as __object__

import py4godot.classes.core as __core__
from py4godot.signals import BuiltinSignal
import py4godot.classes.NavigationRegion2D as __navigationregion2d__
import py4godot.classes.d as __d__
import py4godot.classes.VSplitContainer as __vsplitcontainer__
import py4godot.classes.B as __b__
import py4godot.classes.EditorCommandPalette as __editorcommandpalette__
import py4godot.classes.ConfirmationDialog as __confirmationdialog__
import py4godot.classes.MultiplayerSpawner as __multiplayerspawner__
import py4godot.classes.GPUParticlesCollisionSDF3D as __gpuparticlescollisionsdf3d__
import py4godot.classes.Joint2D as __joint2d__
import py4godot.classes.ShapeCast2D as __shapecast2d__
import py4godot.classes.NavigationRegion3D as __navigationregion3d__
import py4godot.classes.StatusIndicator as __statusindicator__
import py4godot.classes.OpenXRCompositionLayer as __openxrcompositionlayer__
import py4godot.classes.NavigationAgent3D as __navigationagent3d__
import py4godot.classes.SkeletonModifier3D as __skeletonmodifier3d__
import py4godot.classes.CSGPrimitive3D as __csgprimitive3d__
import py4godot.classes.RemoteTransform3D as __remotetransform3d__
import py4godot.classes.Skeleton2D as __skeleton2d__
import py4godot.classes.CanvasItem as __canvasitem__
import py4godot.classes.Node as __node__
import py4godot.classes.CSGMesh3D as __csgmesh3d__
import py4godot.classes.F as __f__
import py4godot.classes.CSGCombiner3D as __csgcombiner3d__
import py4godot.classes.Generic6DOFJoint3D as __generic6dofjoint3d__
import py4godot.classes.OpenXRCompositionLayerQuad as __openxrcompositionlayerquad__
import py4godot.classes.MultiMeshInstance3D as __multimeshinstance3d__
import py4godot.classes.SpringBoneSimulator3D as __springbonesimulator3d__
import py4godot.classes.GPUParticlesAttractorBox3D as __gpuparticlesattractorbox3d__
import py4godot.classes.Skeleton3D as __skeleton3d__
import py4godot.classes.Node2D as __node2d__
import py4godot.classes.XRController3D as __xrcontroller3d__
import py4godot.classes.Separator as __separator__
import py4godot.classes.OccluderInstance3D as __occluderinstance3d__
import py4godot.classes.ScriptEditorBase as __scripteditorbase__
import py4godot.classes.Camera3D as __camera3d__
import py4godot.classes.SpringBoneCollision3D as __springbonecollision3d__
import py4godot.classes.GPUParticlesCollisionBox3D as __gpuparticlescollisionbox3d__
import py4godot.classes.ReflectionProbe as __reflectionprobe__
import py4godot.classes.EditorToaster as __editortoaster__
import py4godot.classes.MeshInstance2D as __meshinstance2d__
import py4godot.classes.PhysicalBone3D as __physicalbone3d__
import py4godot.classes.CSGPolygon3D as __csgpolygon3d__
import py4godot.classes.ScrollContainer as __scrollcontainer__
import py4godot.classes.CharacterBody3D as __characterbody3d__
import py4godot.classes.DirectionalLight3D as __directionallight3d__
import py4godot.classes.DirectionalLight2D as __directionallight2d__
import py4godot.classes.GridMapEditorPlugin as __gridmapeditorplugin__
import py4godot.classes.XRAnchor3D as __xranchor3d__
import py4godot.classes.RigidBody2D as __rigidbody2d__
import py4godot.classes.ParallaxLayer as __parallaxlayer__
import py4godot.classes.SubViewportContainer as __subviewportcontainer__
import py4godot.classes.S as __s__
import py4godot.classes.XRNode3D as __xrnode3d__
import py4godot.classes.AnimatableBody2D as __animatablebody2d__
import py4godot.classes.TouchScreenButton as __touchscreenbutton__
import py4godot.classes.MeshInstance3D as __meshinstance3d__
import py4godot.classes.SpinBox as __spinbox__
import py4godot.classes.EditorPlugin as __editorplugin__
import py4godot.classes.CollisionObject3D as __collisionobject3d__
import py4godot.classes.VisibleOnScreenNotifier2D as __visibleonscreennotifier2d__
import py4godot.classes.CheckBox as __checkbox__
import py4godot.classes.Light2D as __light2d__
import py4godot.classes.l as __l__
import py4godot.classes.GrooveJoint2D as __groovejoint2d__
import py4godot.classes.FogVolume as __fogvolume__
import py4godot.classes.RayCast2D as __raycast2d__
import py4godot.classes.Node3D as __node3d__
import py4godot.classes.RootMotionView as __rootmotionview__
import py4godot.classes.VehicleBody3D as __vehiclebody3d__
import py4godot.classes.VisualInstance3D as __visualinstance3d__
import py4godot.classes.AnimatedSprite3D as __animatedsprite3d__
import py4godot.classes.ShapeCast3D as __shapecast3d__
import py4godot.classes.MenuBar as __menubar__
import py4godot.classes.HingeJoint3D as __hingejoint3d__
import py4godot.classes.EditorFileSystem as __editorfilesystem__
import py4godot.classes.AcceptDialog as __acceptdialog__
import py4godot.classes.Range as __range__
import py4godot.classes.FileSystemDock as __filesystemdock__
import py4godot.classes.Tree as __tree__
import py4godot.classes.ProgressBar as __progressbar__
import py4godot.classes.EditorSpinSlider as __editorspinslider__
import py4godot.classes.OpenXRCompositionLayerCylinder as __openxrcompositionlayercylinder__
import py4godot.classes.CSGBox3D as __csgbox3d__
import py4godot.classes.OptionButton as __optionbutton__
import py4godot.classes.CodeEdit as __codeedit__
import py4godot.classes.t as __t__
import py4godot.classes.PinJoint3D as __pinjoint3d__
import py4godot.classes.SpringBoneCollisionPlane3D as __springbonecollisionplane3d__
import py4godot.classes.ScriptEditor as __scripteditor__
import py4godot.classes.GPUParticlesAttractorVectorField3D as __gpuparticlesattractorvectorfield3d__
import py4godot.classes.h as __h__
import py4godot.classes.ReferenceRect as __referencerect__
import py4godot.classes.LightmapProbe as __lightmapprobe__
import py4godot.classes.OpenXRBindingModifierEditor as __openxrbindingmodifiereditor__
import py4godot.classes.ColorRect as __colorrect__
import py4godot.classes.CenterContainer as __centercontainer__
import py4godot.classes.Label as __label__
import py4godot.classes.P as __p__
import py4godot.classes.TabBar as __tabbar__
import py4godot.classes.GPUParticlesCollisionHeightField3D as __gpuparticlescollisionheightfield3d__
import py4godot.classes.Sprite3D as __sprite3d__
import py4godot.classes.SpriteBase3D as __spritebase3d__
import py4godot.classes.u as __u__
import py4godot.classes.PinJoint2D as __pinjoint2d__
import py4godot.classes.OpenXRInteractionProfileEditorBase as __openxrinteractionprofileeditorbase__
import py4godot.classes.SpringArm3D as __springarm3d__
import py4godot.classes.i as __i__
import py4godot.classes.ColorPickerButton as __colorpickerbutton__
import py4godot.classes.OpenXRVisibilityMask as __openxrvisibilitymask__
import py4godot.classes.BoxContainer as __boxcontainer__
import py4godot.classes.ConeTwistJoint3D as __conetwistjoint3d__
import py4godot.classes.Marker3D as __marker3d__
import py4godot.classes.CSGCylinder3D as __csgcylinder3d__
import py4godot.classes.AnimationPlayer as __animationplayer__
import py4godot.classes.w as __w__
import py4godot.classes.GridContainer as __gridcontainer__
import py4godot.classes.NinePatchRect as __ninepatchrect__
import py4godot.classes.Path3D as __path3d__
import py4godot.classes.o as __o__
import py4godot.classes.k as __k__
import py4godot.classes.LightOccluder2D as __lightoccluder2d__
import py4godot.classes.Timer as __timer__
import py4godot.classes.SpotLight3D as __spotlight3d__
import py4godot.classes.SubViewport as __subviewport__
import py4godot.classes.RetargetModifier3D as __retargetmodifier3d__
import py4godot.classes.BaseButton as __basebutton__
import py4godot.classes.PanelContainer as __panelcontainer__
import py4godot.classes.CanvasLayer as __canvaslayer__
import py4godot.classes.VehicleWheel3D as __vehiclewheel3d__
import py4godot.classes.BoneAttachment3D as __boneattachment3d__
import py4godot.classes.A as __a__
import py4godot.classes.AudioListener3D as __audiolistener3d__
import py4godot.classes.VisibleOnScreenEnabler2D as __visibleonscreenenabler2d__
import py4godot.classes.NavigationObstacle2D as __navigationobstacle2d__
import py4godot.classes.CollisionObject2D as __collisionobject2d__
import py4godot.classes.Area3D as __area3d__
import py4godot.classes.Camera2D as __camera2d__
import py4godot.classes.p as __p__
import py4godot.classes.CSGTorus3D as __csgtorus3d__
import py4godot.classes.G as __g__
import py4godot.classes.a as __a__
import py4godot.classes.M as __m__
import py4godot.classes.XRBodyModifier3D as __xrbodymodifier3d__
import py4godot.classes.NavigationObstacle3D as __navigationobstacle3d__
import py4godot.classes.VisibleOnScreenNotifier3D as __visibleonscreennotifier3d__
import py4godot.classes.RayCast3D as __raycast3d__
import py4godot.classes.LinkButton as __linkbutton__
import py4godot.classes.MenuButton as __menubutton__
import py4godot.classes.ShaderGlobalsOverride as __shaderglobalsoverride__
import py4godot.classes.V as __v__
import py4godot.classes.VideoStreamPlayer as __videostreamplayer__
import py4godot.classes.CanvasModulate as __canvasmodulate__
import py4godot.classes.CollisionPolygon2D as __collisionpolygon2d__
import py4godot.classes.BackBufferCopy as __backbuffercopy__
import py4godot.classes.ItemList as __itemlist__
import py4godot.classes.Area2D as __area2d__
import py4godot.classes.Button as __button__
import py4godot.classes.GraphFrame as __graphframe__
import py4godot.classes.CollisionShape3D as __collisionshape3d__
import py4godot.classes.NavigationAgent2D as __navigationagent2d__
import py4godot.classes.CSGShape3D as __csgshape3d__
import py4godot.classes.Bone2D as __bone2d__
import py4godot.classes.RichTextLabel as __richtextlabel__
import py4godot.classes.N as __n__
import py4godot.classes.NavigationLink3D as __navigationlink3d__
import py4godot.classes.C as __c__
import py4godot.classes.D as __d__
import py4godot.classes.PathFollow2D as __pathfollow2d__
import py4godot.classes.ScrollBar as __scrollbar__
import py4godot.classes.XRCamera3D as __xrcamera3d__
import py4godot.classes.GPUParticles2D as __gpuparticles2d__
import py4godot.classes.Control as __control__
import py4godot.classes.SoftBody3D as __softbody3d__
import py4godot.classes.SkeletonIK3D as __skeletonik3d__
import py4godot.classes.OpenXRInteractionProfileEditor as __openxrinteractionprofileeditor__
import py4godot.classes.TextureButton as __texturebutton__
import py4godot.classes.TileMap as __tilemap__
import py4godot.classes.TextureRect as __texturerect__
import py4godot.classes.HTTPRequest as __httprequest__
import py4godot.classes.AudioStreamPlayer3D as __audiostreamplayer3d__
import py4godot.classes.HSeparator as __hseparator__
import py4godot.classes.Polygon2D as __polygon2d__
import py4godot.classes.GLTFNode as __gltfnode__
import py4godot.classes.Decal as __decal__
import py4godot.classes.AspectRatioContainer as __aspectratiocontainer__
import py4godot.classes.AnimationMixer as __animationmixer__
import py4godot.classes.PhysicalBone2D as __physicalbone2d__
import py4godot.classes.CPUParticles2D as __cpuparticles2d__
import py4godot.classes.EditorResourcePicker as __editorresourcepicker__
import py4godot.classes.WorldEnvironment as __worldenvironment__
import py4godot.classes.CSGSphere3D as __csgsphere3d__
import py4godot.classes.Sprite2D as __sprite2d__
import py4godot.classes.Path2D as __path2d__
import py4godot.classes.DampedSpringJoint2D as __dampedspringjoint2d__
import py4godot.classes.PathFollow3D as __pathfollow3d__
import py4godot.classes.AudioStreamPlayer as __audiostreamplayer__
import py4godot.classes.XRHandModifier3D as __xrhandmodifier3d__
import py4godot.classes.VSlider as __vslider__
import py4godot.classes.ColorPicker as __colorpicker__
import py4godot.classes.GraphEdit as __graphedit__
import py4godot.classes.ResourcePreloader as __resourcepreloader__
import py4godot.classes.2 as __2__
import py4godot.classes.EditorScriptPicker as __editorscriptpicker__
import py4godot.classes.MissingNode as __missingnode__
import py4godot.classes.f as __f__
import py4godot.classes.y as __y__
import py4godot.classes.FileDialog as __filedialog__
import py4godot.classes.Joint3D as __joint3d__
import py4godot.classes.GridMap as __gridmap__
import py4godot.classes.XROrigin3D as __xrorigin3d__
import py4godot.classes.AudioListener2D as __audiolistener2d__
import py4godot.classes.Light3D as __light3d__
import py4godot.classes.AnimatableBody3D as __animatablebody3d__
import py4godot.classes.Window as __window__
import py4godot.classes.EditorFileDialog as __editorfiledialog__
import py4godot.classes.GPUParticles3D as __gpuparticles3d__
import py4godot.classes.GPUParticlesAttractorSphere3D as __gpuparticlesattractorsphere3d__
import py4godot.classes.NavigationLink2D as __navigationlink2d__
import py4godot.classes.RigidBody3D as __rigidbody3d__
import py4godot.classes.GeometryInstance3D as __geometryinstance3d__
import py4godot.classes.Panel as __panel__
import py4godot.classes.TextEdit as __textedit__
import py4godot.classes.HBoxContainer as __hboxcontainer__
import py4godot.classes.Resource as __resource__
import py4godot.classes.EditorInspector as __editorinspector__
import py4godot.classes.HSplitContainer as __hsplitcontainer__
import py4godot.classes.MultiplayerSynchronizer as __multiplayersynchronizer__
import py4godot.classes.Viewport as __viewport__
import py4godot.classes.EditorResourcePreview as __editorresourcepreview__
import py4godot.classes.PhysicalBoneSimulator3D as __physicalbonesimulator3d__
import py4godot.classes.LookAtModifier3D as __lookatmodifier3d__
import py4godot.classes.Marker2D as __marker2d__
import py4godot.classes.Popup as __popup__
import py4godot.classes.GPUParticlesCollision3D as __gpuparticlescollision3d__
import py4godot.classes.Container as __container__
import py4godot.classes.CollisionPolygon3D as __collisionpolygon3d__
import py4godot.classes.c as __c__
import py4godot.classes.HScrollBar as __hscrollbar__
import py4godot.classes.RemoteTransform2D as __remotetransform2d__
import py4godot.classes.ImporterMeshInstance3D as __importermeshinstance3d__
import py4godot.classes.Label3D as __label3d__
import py4godot.classes.CharacterBody2D as __characterbody2d__
import py4godot.classes.InstancePlaceholder as __instanceplaceholder__
import py4godot.classes.OpenXRHand as __openxrhand__
import py4godot.classes.GPUParticlesAttractor3D as __gpuparticlesattractor3d__
import py4godot.classes.LightmapGI as __lightmapgi__
import py4godot.classes.HSlider as __hslider__
import py4godot.classes.r as __r__
import py4godot.classes.StaticBody3D as __staticbody3d__
import py4godot.classes.m as __m__
import py4godot.classes.AudioStreamPlayer2D as __audiostreamplayer2d__
import py4godot.classes.XRFaceModifier3D as __xrfacemodifier3d__
import py4godot.classes.CollisionShape2D as __collisionshape2d__
import py4godot.classes.PointLight2D as __pointlight2d__
import py4godot.classes.CheckButton as __checkbutton__
import py4godot.classes.VoxelGI as __voxelgi__
import py4godot.classes.x as __x__
import py4godot.classes.OpenXRCompositionLayerEquirect as __openxrcompositionlayerequirect__
import py4godot.classes.Parallax2D as __parallax2d__
import py4godot.classes.Line2D as __line2d__
import py4godot.classes.VSeparator as __vseparator__
import py4godot.classes.ParallaxBackground as __parallaxbackground__
import py4godot.classes.GraphNode as __graphnode__
import py4godot.classes.GraphElement as __graphelement__
import py4godot.classes.T as __t__
import py4godot.classes.VScrollBar as __vscrollbar__
import py4godot.classes.LineEdit as __lineedit__
import py4godot.classes.MultiMeshInstance2D as __multimeshinstance2d__
import py4godot.classes.g as __g__
import py4godot.classes.CanvasGroup as __canvasgroup__
import py4godot.classes.GPUParticlesCollisionSphere3D as __gpuparticlescollisionsphere3d__
import py4godot.classes.L as __l__
import py4godot.classes.TileMapLayer as __tilemaplayer__
import py4godot.classes.TabContainer as __tabcontainer__
import py4godot.classes.SliderJoint3D as __sliderjoint3d__
import py4godot.classes.SpringBoneCollisionCapsule3D as __springbonecollisioncapsule3d__
import py4godot.classes.OmniLight3D as __omnilight3d__
import py4godot.classes.n as __n__
import py4godot.classes.TextureProgressBar as __textureprogressbar__
import py4godot.classes.PhysicsBody3D as __physicsbody3d__
import py4godot.classes.AnimatedSprite2D as __animatedsprite2d__
import py4godot.classes.s as __s__
import py4godot.classes.SpringBoneCollisionSphere3D as __springbonecollisionsphere3d__
import py4godot.classes.e as __e__
import py4godot.classes.FlowContainer as __flowcontainer__
import py4godot.classes.CPUParticles3D as __cpuparticles3d__
import py4godot.classes.EditorProperty as __editorproperty__
import py4godot.classes.PhysicsBody2D as __physicsbody2d__
import py4godot.classes.AnimationTree as __animationtree__
import py4godot.classes.VisibleOnScreenEnabler3D as __visibleonscreenenabler3d__
import py4godot.classes.VFlowContainer as __vflowcontainer__
import py4godot.classes.SplitContainer as __splitcontainer__
import py4godot.classes.MarginContainer as __margincontainer__
import py4godot.classes.StaticBody2D as __staticbody2d__
import py4godot.classes.HFlowContainer as __hflowcontainer__
import py4godot.classes.VBoxContainer as __vboxcontainer__
import py4godot.classes.PopupMenu as __popupmenu__
import py4godot.classes.Slider as __slider__
import py4godot.classes.PopupPanel as __popuppanel__
import py4godot.classes.ScriptCreateDialog as __scriptcreatedialog__



class GLTFState(__resource__.Resource):

  @staticmethod
  def constructor()->GLTFState:pass



  @staticmethod
  def cast(other:__object__.Object)->GLTFState:pass


  @property
  def json(self)->__core__.Dictionary: pass
  @json.setter
  def json(self,  value:__core__.Dictionary)->None: pass
  @property
  def major_version(self)->int: pass
  @major_version.setter
  def major_version(self,  value:int)->None: pass
  @property
  def minor_version(self)->int: pass
  @minor_version.setter
  def minor_version(self,  value:int)->None: pass
  @property
  def copyright(self)->str: pass
  @copyright.setter
  def copyright(self,  value:str)->None: pass
  @property
  def glb_data(self)->__core__.PackedByteArray: pass
  @glb_data.setter
  def glb_data(self,  value:__core__.PackedByteArray)->None: pass
  @property
  def use_named_skin_binds(self)->bool: pass
  @use_named_skin_binds.setter
  def use_named_skin_binds(self,  value:bool)->None: pass
  @property
  def nodes(self)->__core__.Array: pass
  @nodes.setter
  def nodes(self,  value:__core__.Array)->None: pass
  @property
  def buffers(self)->__core__.Array: pass
  @buffers.setter
  def buffers(self,  value:__core__.Array)->None: pass
  @property
  def buffer_views(self)->__core__.Array: pass
  @buffer_views.setter
  def buffer_views(self,  value:__core__.Array)->None: pass
  @property
  def accessors(self)->__core__.Array: pass
  @accessors.setter
  def accessors(self,  value:__core__.Array)->None: pass
  @property
  def meshes(self)->__core__.Array: pass
  @meshes.setter
  def meshes(self,  value:__core__.Array)->None: pass
  @property
  def materials(self)->__core__.Array: pass
  @materials.setter
  def materials(self,  value:__core__.Array)->None: pass
  @property
  def scene_name(self)->str: pass
  @scene_name.setter
  def scene_name(self,  value:str)->None: pass
  @property
  def base_path(self)->str: pass
  @base_path.setter
  def base_path(self,  value:str)->None: pass
  @property
  def filename(self)->str: pass
  @filename.setter
  def filename(self,  value:str)->None: pass
  @property
  def root_nodes(self)->__core__.PackedInt32Array: pass
  @root_nodes.setter
  def root_nodes(self,  value:__core__.PackedInt32Array)->None: pass
  @property
  def textures(self)->__core__.Array: pass
  @textures.setter
  def textures(self,  value:__core__.Array)->None: pass
  @property
  def texture_samplers(self)->__core__.Array: pass
  @texture_samplers.setter
  def texture_samplers(self,  value:__core__.Array)->None: pass
  @property
  def images(self)->__core__.Array: pass
  @images.setter
  def images(self,  value:__core__.Array)->None: pass
  @property
  def skins(self)->__core__.Array: pass
  @skins.setter
  def skins(self,  value:__core__.Array)->None: pass
  @property
  def cameras(self)->__core__.Array: pass
  @cameras.setter
  def cameras(self,  value:__core__.Array)->None: pass
  @property
  def lights(self)->__core__.Array: pass
  @lights.setter
  def lights(self,  value:__core__.Array)->None: pass
  @property
  def unique_names(self)->__core__.Array: pass
  @unique_names.setter
  def unique_names(self,  value:__core__.Array)->None: pass
  @property
  def unique_animation_names(self)->__core__.Array: pass
  @unique_animation_names.setter
  def unique_animation_names(self,  value:__core__.Array)->None: pass
  @property
  def skeletons(self)->__core__.Array: pass
  @skeletons.setter
  def skeletons(self,  value:__core__.Array)->None: pass
  @property
  def create_animations(self)->bool: pass
  @create_animations.setter
  def create_animations(self,  value:bool)->None: pass
  @property
  def import_as_skeleton_bones(self)->bool: pass
  @import_as_skeleton_bones.setter
  def import_as_skeleton_bones(self,  value:bool)->None: pass
  @property
  def animations(self)->__core__.Array: pass
  @animations.setter
  def animations(self,  value:__core__.Array)->None: pass
  @property
  def handle_binary_image(self)->int: pass
  @handle_binary_image.setter
  def handle_binary_image(self,  value:int)->None: pass
  @property
  def bake_fps(self)->float: pass
  @bake_fps.setter
  def bake_fps(self,  value:float)->None: pass
  def add_used_extension(self, extension_name:str, required:bool)->None: pass

  def append_data_to_buffers(self, data:__core__.PackedByteArray, deduplication:bool)->int: pass

  def append_gltf_node(self, gltf_node:__gltfnode__.GLTFNode, godot_scene_node:__node__.Node, parent_node_index:int)->int: pass

  def get_json(self)->__core__.Dictionary: pass

  def set_json(self, json:__core__.Dictionary)->None: pass

  def get_major_version(self)->int: pass

  def set_major_version(self, major_version:int)->None: pass

  def get_minor_version(self)->int: pass

  def set_minor_version(self, minor_version:int)->None: pass

  def get_copyright(self)->str: pass

  def set_copyright(self, copyright:str)->None: pass

  def get_glb_data(self)->__core__.PackedByteArray: pass

  def set_glb_data(self, glb_data:__core__.PackedByteArray)->None: pass

  def get_use_named_skin_binds(self)->bool: pass

  def set_use_named_skin_binds(self, use_named_skin_binds:bool)->None: pass

  def get_nodes(self)->typedarray::GLTFNode: pass

  def set_nodes(self, nodes:__core__.Array)->None: pass

  def get_buffers(self)->typedarray::PackedByteArray: pass

  def set_buffers(self, buffers:__core__.Array)->None: pass

  def get_buffer_views(self)->typedarray::GLTFBufferView: pass

  def set_buffer_views(self, buffer_views:__core__.Array)->None: pass

  def get_accessors(self)->typedarray::GLTFAccessor: pass

  def set_accessors(self, accessors:__core__.Array)->None: pass

  def get_meshes(self)->typedarray::GLTFMesh: pass

  def set_meshes(self, meshes:__core__.Array)->None: pass

  def get_animation_players_count(self, idx:int)->int: pass

  def get_animation_player(self, idx:int)->__animationplayer__.AnimationPlayer: pass

  def get_materials(self)->typedarray::Material: pass

  def set_materials(self, materials:__core__.Array)->None: pass

  def get_scene_name(self)->str: pass

  def set_scene_name(self, scene_name:str)->None: pass

  def get_base_path(self)->str: pass

  def set_base_path(self, base_path:str)->None: pass

  def get_filename(self)->str: pass

  def set_filename(self, filename:str)->None: pass

  def get_root_nodes(self)->__core__.PackedInt32Array: pass

  def set_root_nodes(self, root_nodes:__core__.PackedInt32Array)->None: pass

  def get_textures(self)->typedarray::GLTFTexture: pass

  def set_textures(self, textures:__core__.Array)->None: pass

  def get_texture_samplers(self)->typedarray::GLTFTextureSampler: pass

  def set_texture_samplers(self, texture_samplers:__core__.Array)->None: pass

  def get_images(self)->typedarray::Texture2D: pass

  def set_images(self, images:__core__.Array)->None: pass

  def get_skins(self)->typedarray::GLTFSkin: pass

  def set_skins(self, skins:__core__.Array)->None: pass

  def get_cameras(self)->typedarray::GLTFCamera: pass

  def set_cameras(self, cameras:__core__.Array)->None: pass

  def get_lights(self)->typedarray::GLTFLight: pass

  def set_lights(self, lights:__core__.Array)->None: pass

  def get_unique_names(self)->typedarray::String: pass

  def set_unique_names(self, unique_names:__core__.Array)->None: pass

  def get_unique_animation_names(self)->typedarray::String: pass

  def set_unique_animation_names(self, unique_animation_names:__core__.Array)->None: pass

  def get_skeletons(self)->typedarray::GLTFSkeleton: pass

  def set_skeletons(self, skeletons:__core__.Array)->None: pass

  def get_create_animations(self)->bool: pass

  def set_create_animations(self, create_animations:bool)->None: pass

  def get_import_as_skeleton_bones(self)->bool: pass

  def set_import_as_skeleton_bones(self, import_as_skeleton_bones:bool)->None: pass

  def get_animations(self)->typedarray::GLTFAnimation: pass

  def set_animations(self, animations:__core__.Array)->None: pass

  def get_scene_node(self, idx:int)->__navigationregion2d__.NavigationRegion2D|__vsplitcontainer__.VSplitContainer|__editorcommandpalette__.EditorCommandPalette|__confirmationdialog__.ConfirmationDialog|__multiplayerspawner__.MultiplayerSpawner|__gpuparticlescollisionsdf3d__.GPUParticlesCollisionSDF3D|__joint2d__.Joint2D|__shapecast2d__.ShapeCast2D|__navigationregion3d__.NavigationRegion3D|__statusindicator__.StatusIndicator|__openxrcompositionlayer__.OpenXRCompositionLayer|__navigationagent3d__.NavigationAgent3D|__skeletonmodifier3d__.SkeletonModifier3D|__csgprimitive3d__.CSGPrimitive3D|__remotetransform3d__.RemoteTransform3D|__skeleton2d__.Skeleton2D|__canvasitem__.CanvasItem|__node__.Node|__csgmesh3d__.CSGMesh3D|__csgcombiner3d__.CSGCombiner3D|__generic6dofjoint3d__.Generic6DOFJoint3D|__openxrcompositionlayerquad__.OpenXRCompositionLayerQuad|__multimeshinstance3d__.MultiMeshInstance3D|__springbonesimulator3d__.SpringBoneSimulator3D|__gpuparticlesattractorbox3d__.GPUParticlesAttractorBox3D|__skeleton3d__.Skeleton3D|__node2d__.Node2D|__xrcontroller3d__.XRController3D|__separator__.Separator|__occluderinstance3d__.OccluderInstance3D|__scripteditorbase__.ScriptEditorBase|__camera3d__.Camera3D|__springbonecollision3d__.SpringBoneCollision3D|__gpuparticlescollisionbox3d__.GPUParticlesCollisionBox3D|__reflectionprobe__.ReflectionProbe|__editortoaster__.EditorToaster|__meshinstance2d__.MeshInstance2D|__physicalbone3d__.PhysicalBone3D|__csgpolygon3d__.CSGPolygon3D|__scrollcontainer__.ScrollContainer|__characterbody3d__.CharacterBody3D|__directionallight3d__.DirectionalLight3D|__directionallight2d__.DirectionalLight2D|__gridmapeditorplugin__.GridMapEditorPlugin|__xranchor3d__.XRAnchor3D|__rigidbody2d__.RigidBody2D|__parallaxlayer__.ParallaxLayer|__subviewportcontainer__.SubViewportContainer|__xrnode3d__.XRNode3D|__animatablebody2d__.AnimatableBody2D|__touchscreenbutton__.TouchScreenButton|__meshinstance3d__.MeshInstance3D|__spinbox__.SpinBox|__editorplugin__.EditorPlugin|__collisionobject3d__.CollisionObject3D|__visibleonscreennotifier2d__.VisibleOnScreenNotifier2D|__checkbox__.CheckBox|__light2d__.Light2D|__groovejoint2d__.GrooveJoint2D|__fogvolume__.FogVolume|__raycast2d__.RayCast2D|__node3d__.Node3D|__rootmotionview__.RootMotionView|__vehiclebody3d__.VehicleBody3D|__visualinstance3d__.VisualInstance3D|__animatedsprite3d__.AnimatedSprite3D|__shapecast3d__.ShapeCast3D|__menubar__.MenuBar|__hingejoint3d__.HingeJoint3D|__editorfilesystem__.EditorFileSystem|__acceptdialog__.AcceptDialog|__range__.Range|__filesystemdock__.FileSystemDock|__tree__.Tree|__progressbar__.ProgressBar|__editorspinslider__.EditorSpinSlider|__openxrcompositionlayercylinder__.OpenXRCompositionLayerCylinder|__csgbox3d__.CSGBox3D|__optionbutton__.OptionButton|__codeedit__.CodeEdit|__pinjoint3d__.PinJoint3D|__springbonecollisionplane3d__.SpringBoneCollisionPlane3D|__scripteditor__.ScriptEditor|__gpuparticlesattractorvectorfield3d__.GPUParticlesAttractorVectorField3D|__referencerect__.ReferenceRect|__lightmapprobe__.LightmapProbe|__openxrbindingmodifiereditor__.OpenXRBindingModifierEditor|__colorrect__.ColorRect|__centercontainer__.CenterContainer|__label__.Label|__tabbar__.TabBar|__gpuparticlescollisionheightfield3d__.GPUParticlesCollisionHeightField3D|__sprite3d__.Sprite3D|__spritebase3d__.SpriteBase3D|__pinjoint2d__.PinJoint2D|__openxrinteractionprofileeditorbase__.OpenXRInteractionProfileEditorBase|__springarm3d__.SpringArm3D|__colorpickerbutton__.ColorPickerButton|__openxrvisibilitymask__.OpenXRVisibilityMask|__boxcontainer__.BoxContainer|__conetwistjoint3d__.ConeTwistJoint3D|__marker3d__.Marker3D|__csgcylinder3d__.CSGCylinder3D|__animationplayer__.AnimationPlayer|__gridcontainer__.GridContainer|__ninepatchrect__.NinePatchRect|__path3d__.Path3D|__subviewport__.SubViewport|__lightoccluder2d__.LightOccluder2D|__timer__.Timer|__spotlight3d__.SpotLight3D|__retargetmodifier3d__.RetargetModifier3D|__basebutton__.BaseButton|__panelcontainer__.PanelContainer|__canvaslayer__.CanvasLayer|__vehiclewheel3d__.VehicleWheel3D|__boneattachment3d__.BoneAttachment3D|__audiolistener3d__.AudioListener3D|__visibleonscreenenabler2d__.VisibleOnScreenEnabler2D|__navigationobstacle2d__.NavigationObstacle2D|__collisionobject2d__.CollisionObject2D|__area3d__.Area3D|__camera2d__.Camera2D|__csgtorus3d__.CSGTorus3D|__xrbodymodifier3d__.XRBodyModifier3D|__navigationobstacle3d__.NavigationObstacle3D|__visibleonscreennotifier3d__.VisibleOnScreenNotifier3D|__raycast3d__.RayCast3D|__linkbutton__.LinkButton|__menubutton__.MenuButton|__shaderglobalsoverride__.ShaderGlobalsOverride|__videostreamplayer__.VideoStreamPlayer|__canvasmodulate__.CanvasModulate|__collisionpolygon2d__.CollisionPolygon2D|__backbuffercopy__.BackBufferCopy|__itemlist__.ItemList|__area2d__.Area2D|__button__.Button|__graphframe__.GraphFrame|__collisionshape3d__.CollisionShape3D|__navigationagent2d__.NavigationAgent2D|__csgshape3d__.CSGShape3D|__bone2d__.Bone2D|__richtextlabel__.RichTextLabel|__navigationlink3d__.NavigationLink3D|__pathfollow2d__.PathFollow2D|__scrollbar__.ScrollBar|__xrcamera3d__.XRCamera3D|__gpuparticles2d__.GPUParticles2D|__control__.Control|__softbody3d__.SoftBody3D|__skeletonik3d__.SkeletonIK3D|__openxrinteractionprofileeditor__.OpenXRInteractionProfileEditor|__texturebutton__.TextureButton|__tilemap__.TileMap|__texturerect__.TextureRect|__httprequest__.HTTPRequest|__audiostreamplayer3d__.AudioStreamPlayer3D|__hseparator__.HSeparator|__polygon2d__.Polygon2D|__decal__.Decal|__aspectratiocontainer__.AspectRatioContainer|__animationmixer__.AnimationMixer|__physicalbone2d__.PhysicalBone2D|__cpuparticles2d__.CPUParticles2D|__editorresourcepicker__.EditorResourcePicker|__worldenvironment__.WorldEnvironment|__csgsphere3d__.CSGSphere3D|__sprite2d__.Sprite2D|__path2d__.Path2D|__dampedspringjoint2d__.DampedSpringJoint2D|__pathfollow3d__.PathFollow3D|__audiostreamplayer__.AudioStreamPlayer|__xrhandmodifier3d__.XRHandModifier3D|__vslider__.VSlider|__colorpicker__.ColorPicker|__graphedit__.GraphEdit|__resourcepreloader__.ResourcePreloader|__editorscriptpicker__.EditorScriptPicker|__missingnode__.MissingNode|__filedialog__.FileDialog|__joint3d__.Joint3D|__gridmap__.GridMap|__xrorigin3d__.XROrigin3D|__audiolistener2d__.AudioListener2D|__light3d__.Light3D|__animatablebody3d__.AnimatableBody3D|__window__.Window|__editorfiledialog__.EditorFileDialog|__gpuparticles3d__.GPUParticles3D|__gpuparticlesattractorsphere3d__.GPUParticlesAttractorSphere3D|__navigationlink2d__.NavigationLink2D|__rigidbody3d__.RigidBody3D|__geometryinstance3d__.GeometryInstance3D|__panel__.Panel|__textedit__.TextEdit|__hboxcontainer__.HBoxContainer|__editorinspector__.EditorInspector|__hsplitcontainer__.HSplitContainer|__multiplayersynchronizer__.MultiplayerSynchronizer|__viewport__.Viewport|__editorresourcepreview__.EditorResourcePreview|__physicalbonesimulator3d__.PhysicalBoneSimulator3D|__lookatmodifier3d__.LookAtModifier3D|__marker2d__.Marker2D|__popup__.Popup|__gpuparticlescollision3d__.GPUParticlesCollision3D|__container__.Container|__collisionpolygon3d__.CollisionPolygon3D|__hscrollbar__.HScrollBar|__remotetransform2d__.RemoteTransform2D|__importermeshinstance3d__.ImporterMeshInstance3D|__label3d__.Label3D|__characterbody2d__.CharacterBody2D|__instanceplaceholder__.InstancePlaceholder|__openxrhand__.OpenXRHand|__gpuparticlesattractor3d__.GPUParticlesAttractor3D|__lightmapgi__.LightmapGI|__hslider__.HSlider|__staticbody3d__.StaticBody3D|__audiostreamplayer2d__.AudioStreamPlayer2D|__xrfacemodifier3d__.XRFaceModifier3D|__collisionshape2d__.CollisionShape2D|__pointlight2d__.PointLight2D|__checkbutton__.CheckButton|__voxelgi__.VoxelGI|__openxrcompositionlayerequirect__.OpenXRCompositionLayerEquirect|__parallax2d__.Parallax2D|__line2d__.Line2D|__vseparator__.VSeparator|__parallaxbackground__.ParallaxBackground|__graphnode__.GraphNode|__graphelement__.GraphElement|__vscrollbar__.VScrollBar|__lineedit__.LineEdit|__multimeshinstance2d__.MultiMeshInstance2D|__canvasgroup__.CanvasGroup|__gpuparticlescollisionsphere3d__.GPUParticlesCollisionSphere3D|__tilemaplayer__.TileMapLayer|__tabcontainer__.TabContainer|__sliderjoint3d__.SliderJoint3D|__springbonecollisioncapsule3d__.SpringBoneCollisionCapsule3D|__omnilight3d__.OmniLight3D|__textureprogressbar__.TextureProgressBar|__physicsbody3d__.PhysicsBody3D|__animatedsprite2d__.AnimatedSprite2D|__springbonecollisionsphere3d__.SpringBoneCollisionSphere3D|__flowcontainer__.FlowContainer|__cpuparticles3d__.CPUParticles3D|__editorproperty__.EditorProperty|__physicsbody2d__.PhysicsBody2D|__animationtree__.AnimationTree|__visibleonscreenenabler3d__.VisibleOnScreenEnabler3D|__vflowcontainer__.VFlowContainer|__splitcontainer__.SplitContainer|__margincontainer__.MarginContainer|__staticbody2d__.StaticBody2D|__hflowcontainer__.HFlowContainer|__vboxcontainer__.VBoxContainer|__popupmenu__.PopupMenu|__slider__.Slider|__popuppanel__.PopupPanel|__scriptcreatedialog__.ScriptCreateDialog: pass

  def get_node_index(self, scene_node:__node__.Node)->int: pass

  def get_additional_data(self, extension_name:__core__.StringName|str)->object: pass

  def set_additional_data(self, extension_name:__core__.StringName|str, additional_data:object)->None: pass

  def get_handle_binary_image(self)->int: pass

  def set_handle_binary_image(self, method:int)->None: pass

  def set_bake_fps(self, value:float)->None: pass

  def get_bake_fps(self)->float: pass


